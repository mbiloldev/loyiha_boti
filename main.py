...
TELEGRAM_TOKEN = "your_tg_bot_token_hero"


# Inline tugmalar menyusi
def build_menu():
    keyboard = [
        [InlineKeyboardButton("📈 Narxni ko‘r", callback_data='show_price')],
        [InlineKeyboardButton("🔮 Prognoz", callback_data='show_forecast')],
        [InlineKeyboardButton("📊 Grafik", callback_data='show_chart')],
        [InlineKeyboardButton("🔔 Ogohlantirish sozlash", callback_data='set_alert')]
    ]
    return InlineKeyboardMarkup(keyboard)


# Ogohlantirish narxini so‘rash uchun klaviatura
def build_alert_input_keyboard():
    keyboard = [
...
def build_cnn_model():
    model = Sequential([
        ...
        Dense(3, activation='softmax')  # ko‘tariladi, tushadi, barqaror
    ])
...
def predict_price(model, data):
    ...
    trend = "ko‘tariladi" if change > 0.1 else "tushadi" if change < -0.1 else "barqaror"
...
def start(update: Update, context: CallbackContext):
    update.message.reply_text(
        ...
        "Tugmalar orqali ma'lumot oling yoki grafik rasm yuboring!",
...
def button(update: Update, context: CallbackContext):
    ...
    if action == 'show_price':
        ...
        if price_data:
            ...
            query.message.reply_text(
                "❌ Narx ma'lumotlari olinmadi. MT5 ulanganligini tekshiring.",
                reply_markup=build_menu()
            )
        ...
        if len(data) < 10:
            query.message.reply_text(
                "❌ Prognoz uchun yetarli ma'lumot yo‘q",
                reply_markup=build_menu()
            )
            return
        ...
        query.message.reply_text(
            "🔔 Ogohlantirish uchun narxni yozing (masalan: 3200)",
            reply_markup=build_alert_input_keyboard()
        )
        ...
        context.user_data.pop('awaiting_alert', None)


def handle_text(update: Update, context: CallbackContext):
    if context.user_data.get('awaiting_alert'):
        try:
            price_level = float(update.message.text)
            context.user_data["alert_price"] = price_level
            update.message.reply_text(
                f"🔔 Narx ${price_level:.2f} ga yetganda xabar beraman",
                reply_markup=build_menu()
            )
            context.user_data.pop('awaiting_alert')
        except ValueError:
            update.message.reply_text(
                "❌ Iltimos, narxni to‘g‘ri kiriting (masalan: 3200)",
                reply_markup=build_alert_input_keyboard()
            )
    else:
        update.message.reply_text(
            "📋 Tugmalardan foydalaning yoki grafik rasm yuboring!",
            reply_markup=build_menu()
        )
...
def handle_image(update: Update, context: CallbackContext):
    ...
    if trend is None:
        update.message.reply_text(
            "❌ Rasmni tahlil qilib bo‘lmadi. Iltimos, aniq grafik yuboring.",
            reply_markup=build_menu()
        )
        return
    update.message.reply_text(
